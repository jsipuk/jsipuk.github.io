...
- [Deployment on this repository (GitHub Pages)](#deployment-on-this-repository-github-pages)
...
- **Browser smoke test** — with the dev server running:
  ...
  deployment: `node scripts/smoke.mjs https://example.com/merge-life-game`
...
2. In Vercel, "Add New… → Project", pick the repository and set the **root
   directory** to `merge-life`.
3. Framework preset: **Next.js**. Build command `npm run build`, output
   directory `out` (Vercel detects both automatically).
4. Deploy. No environment variables are required.

## Deployment on this repository (GitHub Pages)

This repository is a GitHub Pages site, so a built copy of the game is committed
at the repo root in `merge-life-game/` and is served from
`https://jsipuk.github.io/merge-life-game/`. To refresh it after a change:

```bash
cd merge-life
rm -rf .next out
NEXT_PUBLIC_BASE_PATH=/merge-life-game npm run build
rm -rf ../merge-life-game && cp -r out ../merge-life-game
```

Keep the built copy outside `merge-life/` — a folder named `app/` inside the
