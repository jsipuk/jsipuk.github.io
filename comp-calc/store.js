/* Commission Calculator — storage and encryption layer.
 *
 * A copy of ground/store.js, byte-for-byte identical apart from this comment
 * and the APP constant. The duplication is deliberate and matches the reason
 * given in Ground's copy: each app precaches its own files so it works
 * offline, and a service worker only intercepts requests inside its own
 * scope, so a shared /lib/ copy would silently break offline. Ground's
 * test/run.js diffs every copy and fails if they drift apart, so this file
 * must not be edited on its own.
 *
 * Deliberately has no DOM access and never touches localStorage itself, so the
 * same file runs in the browser and under Node. The app layer owns
 * persistence; this file owns the envelope format and the cryptography.
 *
